...
namespace CareerCloud.BusinessLogicLayer
{
    public class CompanyProfileLogic : BaseLogic<CompanyProfilePoco>
    {
        ...
        public CompanyProfileLogic(DataAccessLayer.IDataRepository<CompanyProfilePoco> repository) : base(repository)
        {
        }
            ...
            foreach (CompanyProfilePoco poco in pocos) {
                ...
                if (!String.IsNullOrEmpty(poco.CompanyWebsite) && !_validExtensions.Any(x => poco.CompanyWebsite.EndsWith(x)))
                {
                    exceptions.Add(new ValidationException(600,
                        $"Valid websites must end with the following extensions – \".ca\", \".com\", \".biz\" : Id = ${poco.Id}"));
                }
                    ...
                    )
                {
                    exceptions.Add(new ValidationException(601,
                       $"ContactPhone Must correspond to a valid phone number(e.g. 416 - 555 - 1234) : Id = ${poco.Id}"));
                }
